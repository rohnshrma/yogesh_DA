...
This assignment provides tasks to practice SQL concepts including table creation with primary and foreign keys, data insertion, updates, deletes, and querying with INNER JOINs. You must create and populate tables with at least 50 dummy data rows in each table (`customers` and `purchases`). Use a PostgreSQL database to complete the tasks. Insert queries for the dummy data are provided below.
...
1. Insert at least 50 rows into the `customers` table:
...
**SQL Insert Queries for Dummy Data:**
...
2. Update the `category` column for all purchases where `customer_id` corresponds to customers with `first_name` = 'Alice':
...
5. Update the `birth_year` column for customers with `first_name` = 'Alice':

   - Set `birth_year` to '1995'.
...
2. Perform an INNER JOIN to combine the `customers` and `purchases` tables:
   ...
   - Use table aliases (`c` for `customers`, `p` for `purchases`) for brevity.
   - Only include rows where there is a match between the tables.

3. Query rows from `customers` where `first_name` is 'Alice':
...
11. Query rows from `customers` where `first_name` is in ('Alice', 'Bob', 'Clara'):

    - Select all columns.

12. Query rows from `customers` where `first_name` is 'Alice' AND `email` is 'alice.brown@email.com':

    - Select all columns.

13. Query rows from `customers` where `first_name` is 'Alice' OR `email` is 'bob.smith@email.com':
    - Select all columns.
...
1. Delete all rows from `customers` where `first_name` is 'Alice':
...
- The provided insert queries create 50 rows in each table with diverse data and appropriate constraints (e.g., NULL emails, varied purchase dates).
