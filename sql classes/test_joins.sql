...
-- Task 18: Select all residents, including those without employment records, using RIGHT JOIN with 'residents' as right table
...
-- Task 30: Calculate the maximum salary for employment records where region_id is not NULL (join with 'residents')
-- Task 31: Calculate the minimum record_date for employment records where occupation_id is in (1, 2)
...
-- Task 40: Select resident_name and occupation_name for records where record_date is in April 2025 using INNER JOIN
-- Task 41: Select all regions and their resident count using LEFT JOIN and GROUP BY
...
-- Task 49: Select all columns from 'occupations' where occupation_name contains 'er'
-- Task 50: Calculate the total number of employment records per state using LEFT JOIN and GROUP BY